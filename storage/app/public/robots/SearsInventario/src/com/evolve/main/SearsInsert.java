...
public class SearsInsert {
	...
	public static void insertarBD(String cuenta) {
		...
		try {
			...
			ResultSet rs = s.executeQuery ("SELECT fechaArchivo AS fecha FROM tempsearsinventarios GROUP BY fechaArchivo ORDER BY fechaArchivo DESC LIMIT 1 ;");
        ...
        for (int k = 0; k < oldDates.size(); k++) {
            ...
            try {
                ...
                util.renameFileInventarios(fecha, config);
	...
	public static void readDataJuguetron(String fecha) {
        ...
        try {
            ...
            String urlFile = pathFiles+portal+"\\inventarios\\"+prop.getProperty(portal+".prefix")+fecha + ".csv";
            File csvFile = new File(urlFile);                      
            
            loadCSVJuguetronCesarfer(urlFile, "tempsearsinventarios", true, headerRow, isDoble, fecha);
